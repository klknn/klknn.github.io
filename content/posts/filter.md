...
各種変換により、デジタルの実装（離散時間領域）と、位相周波数応答の特性（フーリエ領域）を定義することができる。ここでは触れないが、一時的にでてくるラプラス変換もアナログ回路との対応があり勉強すると楽しい。
...
ここまでの話を 4 から 1 に逆にたどると、つまるところデジタルフィルタの設計の流れとなる。つまり所望の位相・周波数特性をもつフィルタ係数 $(a_t, b_t)$ をいか定義するかという作業である。
